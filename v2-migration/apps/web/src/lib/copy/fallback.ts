import type { ProductId, CustomerSegmentId } from '@salesarts/shared/intake-spec.types';

type SegmentMap = Partial<Record<CustomerSegmentId, string[]>> & { default: string[] };

/**
 * Pre-written headlines when the Anthropic API is offline or in dev.
 * Every line passes the brand voice check: honest, concrete, no fluff.
 */
export const FALLBACK_HEADLINES: Record<ProductId, SegmentMap> = {
  solar_pr: {
